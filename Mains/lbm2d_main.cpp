...
int main() {
	...
	for (int x = 0; x < simulation_resolution.x; x++) {
		for (int y = 0; y < simulation_resolution.y; y++) {
			if (glm::distance(glm::vec2(x, y), glm::vec2(simulation_resolution) / glm::vec2(4, 2)) < 13)
				lbm2d_solver.set_boundry(glm::ivec2(x, y), true);
		}
	}

	lbm2d_solver.set_population(1.0f);
	lbm2d_solver.add_random_population(0.3f);
	lbm2d_solver.set_population(1, 2.3f);
	...
	while (!window.should_close()) {
		...
		lbm2d_solver.iterate_time(std::chrono::duration<double, std::milli>(deltatime * 32));
		...
		if (display_mode == 1)
			...
			lbm2d_solver.copy_to_texture_velocity_magnetude(texture);
